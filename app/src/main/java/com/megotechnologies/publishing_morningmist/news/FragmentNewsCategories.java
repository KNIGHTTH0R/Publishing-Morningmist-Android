package com.megotechnologies.publishing_morningmist.news;
...
public class FragmentNewsCategories extends FragmentMeta implements ZCFragmentLifecycle, ZCRunnable {
	...
	void loadFromLocalDB() {
		...
		llContainer.setPadding(0, 0, 0, MainActivity.SPACING);
		...
		for(int i = MainActivity.NUM_INIT_STREAMS; i < records.size();) {
			...
			if(i == MainActivity.NUM_INIT_STREAMS) {
				...
				rlRow.setBackgroundColor(activity.getResources().getColor(R.color.white));
				...
				rlRow.setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View v) {
						...
						if (!activity.IS_CLICKED) {
							...
							FragmentNewsItemsList frag = new FragmentNewsItemsList();
