...
import com.megotechnologies.publishing_morningmist.push.NotificationService;
...
public class FragmentLoading extends FragmentMeta implements ZCFragmentLifecycle, ZCRunnable {
	...
	public void run() {
		...
		while(RUN_FLAG) {
			...
			if(threadStream.getState().name().equals(MainActivity.TH_STATE_TERM) && (currTime - threadStartTime) > MainActivity.TH_SPLASH_MIN_DURATION) {
				RUN_FLAG = false;
			}
